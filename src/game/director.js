...
export class Director{
  ...
  // Progress through the contract, 0..1.
  ...
  get enemyCap(){
    ...
    const base=performance?90:165;
    // Quadratic ramp: the opening minutes stay readable while the player is
    // still under-levelled, and saturation only arrives in the back half.
    const progress=this.progress;
    const ramp=.12+progress*progress*.88;
    ...
    return Math.round(base*ramp*this.difficulty.densityMult*bossThinning);
  }
  ...
  // Archetypes legal at the current point in the run.
  availableArchetypes(){
    const tierCap=Math.floor(this.progress*5)+(this.difficulty.id>=3?1:0);
  ...
  update(dt){
    ...
    this.eliteTimer-=dt;
    if(this.eliteTimer<=0&&this.progress>.22){
      this.spawnElite();
      this.eliteTimer=clamp(46-this.progress*30,10,46)/this.difficulty.densityMult;
    }
  }
  ...
  queueWave(){
    ...
    const size=Math.round(
      (4+progress*progress*26+this.waveIndex*.45)*this.difficulty.densityMult*(1+this.pressure*.3)
    );
  ...
  queueSurge(){
    const rng=this.engine.rng;
    const size=Math.round((14+this.progress*30)*this.difficulty.densityMult);
    const squad=new Squad('assault');
    const bearing=rng.angle();
  ...
  spawnElite(){
    const engine=this.engine;
    const rng=engine.rng;
    const tierCap=Math.min(ELITES.length,2+Math.floor(this.progress*ELITES.length));
